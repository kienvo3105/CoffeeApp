...
const ExchangeCoinItem = ({ item, handleChangeCoupon }) => {
    const { image, costChange, discount, id } = item;
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{ marginHorizontal: 5, flex: 1 }}>
                <Text style={{ fontSize: 15, fontWeight: 'bold', color: colors.textPrimary }}>{costChange} COINS = {formatCurrency(discount)}</Text>
                <Text style={{ fontSize: 13, color: colors.darkGray, backgroundColor: colors.background, width: 80, height: 20, textAlignVertical: 'center', textAlign: 'center', borderRadius: 20, marginVertical: 5 }}>{costChange} coins</Text>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Pressable onPress={() => handleChangeCoupon(numberProduct, id, costChange)}>
